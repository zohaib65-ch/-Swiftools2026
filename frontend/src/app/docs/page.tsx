...
import { Code, Image, Palette, FileText, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function DocumentationPage() {
  ...
  const quickTools = Cardsections[3].tools.sort(() => Math.random() - 0.5).slice(0, 5);
  ...
  return (
    <div className="bg-linear-to-r from-[#f8f7ff] via-[#fff7f7] to-[#fffdf5]">
      <div className="flex min-h-screen bg-linear-to-r from-[#f8f7ff] via-[#fff7f7] to-[#fffdf5]">
          ...
          <div className={`flex-1 overflow-y-auto ${sidebarOpen ? "px-3 py-4" : "py-4"} ${!sidebarOpen && !isMobile ? "flex flex-col items-center" : ""}`}>
            ...
            {sidebarOpen && (
              <div className="py-4 px-5 border-b border-gray-200">
                <h2 className=" mt-16 w-auto text-lg font-semibold text-black uppercase tracking-wide">Introduction</h2>
              </div>
            )}
              ...
              {categories.map((cat) => (
                <div key={cat.id} className="w-full">
                  <button
                    ...
                    } rounded-xl font-medium transition-all duration-200 cursor-pointer ${activeCategory === cat.id ? "bg-gray-50 shadow-inner" : "text-black "}`}
                    aria-expanded={activeCategory === cat.id}
                  >
                    <div className="flex items-center gap-2">
                      {cat.icon}
                      {sidebarOpen && <span className="text-base font-medium">{cat.name}</span>}
                    </div>
                    {sidebarOpen && <span className="text-xs text-black">{cat.count}</span>}
                  </button>
                        ...
                        .map((tool) => (
                          <button
                            ...
                            className={`flex items-center gap-2 px-2 text-xs py-2 rounded-lg transition-all duration-150 cursor-pointer ${
                              selectedTool === tool.id ? " bg-gray-50  text-black font-medium" : "   text-black"
                            }`}
                          >
        ...
        {/* Main Content */}
        <main className={`flex-1 sm:p-8 p-1  transition-all duration-300 ${sidebarOpen && !isMobile ? "mt-24" : ""}`}>
          <Navbar />
          ...
          {selectedTool ? (
            <div className="rounded-2xl px-6 mt-2   ">
              ...
              <div className="space-y-1">
                ...
                <p className="text-gray-500 text-sm sm:text-md max-w-3xl">{activeTool.description}</p>
              </div>
                ...
                <div className="p-6 mt-2  mb-3 rounded-xl bg-white backdrop-blur-sm">
                  ...
                  <ul className="space-y-0">
                    {activeTool.features.map((feature, idx) => (
                      <li key={idx} className="flex items-start gap-0.5  rounded-lg hover:bg-gray-100/50">
              ...
              {activeTool.apiEndpoint && (
                <div className="max-w-2xl mt-2">
                  <h3 className="text-lg font-semibold  text-gray-900 uppercase mb-1">API Endpoint</h3>
                  <div className="flex items-center gap-3 rounded-lg border border-gray-200 bg-white p-2 shadow-sm">
                    <p className="font-mono text-sm text-gray-800 truncate flex-1">{activeTool.apiEndpoint}</p>
                    <button className="bg-gray-200 px-3 py-2 text-black cursor-pointer rounded text-xs hover:bg-gray-300" onClick={() => navigator.clipboard.writeText(activeTool.apiEndpoint)}>
                      Copy
                    </button>
                  </div>
                </div>
              ...
              {activeTool.requestFormat && (
                <div className=" rounded-xl mt-2 bg-gray-50/50 backdrop-blur-sm">
                  ...
                  <ul className="space-y-1">
                    {Object.entries(activeTool.requestFormat.fields).map(([key, value]) => (
            ...
            <section className={`${sidebarOpen && !isMobile ? "mt-11" : ""}  flex min-h-screen flex-col items-start px-4 text-left max-w-6xl mx-auto `}>
              {/* Badge */}
              <div className="mb-4">
                <span className="text-black rounded-full bg-white/80 backdrop-blur px-5 py-1.5 text-sm font-medium shadow-md border border-gray-200">✨ Introducing SwiftTools 1.0</span>
              </div>

              {/* Main Heading */}
              <h1 className="max-w-4xl text-3xl font-semibold leading-tight text-black md:text-5xl">Welcome to the Developer Portal</h1>

              {/* Subheading */}
              <p className="mt-6 max-w-2xl text-base text-gray-600 md:text-lg">
                Everything you need to build, integrate, and scale with SwiftTools. Clean APIs, powerful tools, and developer-first documentation.
              </p>
              ...
              <div className="mt-12 max-w-3xl space-y-6 text-left">
                <h2 className="text-xl font-semibold text-gray-800 md:text-2xl">Introduction</h2>
                <p className="text-gray-600 leading-relaxed text-md">
                  SwiftTools is your all-in-one digital toolbox designed to simplify everyday workflows. Fast, secure, privacy-focused, and built for performance — everything you need in one platform.
                </p>
              </div>

              {/* Base URL Card */}
              <div className="mt-10 w-full max-w-2xl">
                <h3 className="text-xl  font-semibold text-gray-800  tracking-wide mb-3">Base URL</h3>
                <div className="relative group">
                  <div className="flex items-center gap-3 rounded-lg border border-gray-200 bg-white/90 backdrop-blur shadow-lg p-4 pl-5 hover:border-indigo-300 transition-all duration-200">
                    <div className="flex-1 min-w-0">
                      <p className="font-mono text-gray-800 text-sm md:text-base truncate">https://swift-tools-lemon.vercel.app/</p>
                    </div>
                    <button
                      className="text-black bg-gray-200 cursor-pointer rounded-lg p-2 text-xs hover:bg-gray-300"
                      onClick={() => {
                        navigator.clipboard.writeText("https://swift-tools-lemon.vercel.app/");
                      }}
                    >
                      <span>Copy URL</span>
                    </button>
                  ...
                  <div className="absolute -top-8 left-0 opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none">
                    <div className="bg-gray-800 text-white text-xs py-1 px-2 rounded">Click to copy</div>
                  </div>
                </div>
                <p className="mt-3 text-xs text-gray-500">Use this URL as the base for all API endpoints</p>
              </div>

              {/* Quick Tools */}
              <h3 className="text-xl text-center sm:text-left font-semibold text-gray-800 mt-8 mb-4">Quick Tools</h3>
                ...
                {quickTools.map((tool) => (
                  ...
                  >
                    <div className="flex items-center gap-3 mb-3">
                      <div className="flex items-center justify-center w-10 h-10 rounded-lg bg-indigo-50 text-indigo-600">{iconMap[tool.name] ?? "⚙️"}</div>

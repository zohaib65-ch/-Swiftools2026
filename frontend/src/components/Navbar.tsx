  ...
  return (
    <header className="absolute top-3 left-0 right-0 z-40">
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        <div className="flex items-center justify-between h-16 rounded-full bg-white/70 backdrop-blur-xl shadow-lg shadow-black/5 border border-white/20 px-6 transition-all duration-300 hover:shadow-xl hover:bg-white/80 hover:scale-[1.005]">
          ...
          <div className="hidden md:flex items-center gap-4">
            {!session?.user ? (
              <>
                <button onClick={() => router.push('/login')} className="text-sm font-medium text-gray-700 hover:text-black transition-colors">Log in</button>
                <button
                  onClick={() => router.push('/signup')}
                  className="rounded-full bg-black px-6 py-2.5 text-sm font-semibold text-white hover:bg-gray-800 transition-all shadow-md hover:shadow-lg hover:-translate-y-0.5"
                >
                  Register
                </button>
              </>
              ...
              <div className="flex items-center gap-4">
                ...
                {/* User Avatar */}
                <Link href="/dashboard" className="w-10 h-10 rounded-full bg-linear-to-br from-gray-800 to-black text-white flex items-center justify-center text-sm font-bold shadow-md hover:ring-2 ring-gray-200 transition-all">
                  {userInitial}
                </Link>
              </div>
            )}
      ...
      <div className={`fixed top-0 left-0 w-full h-full z-40 transition-opacity duration-300 ${isOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
        ...
        {/* Sidebar */}
        <div className={`flex flex-col justify-between fixed top-0 left-0 h-full w-72 bg-white/90 backdrop-blur-2xl shadow-2xl p-6 transition-transform duration-300 border-r border-white/20 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
            ...
            {!session?.user ? (
              ...
              <div className="flex flex-col gap-3">
                ...
                <Link href="/dashboard" className="flex items-center gap-3 p-3 rounded-xl bg-gray-50 hover:bg-gray-100 transition-colors border border-gray-100">
                  <span className="w-10 h-10 rounded-full bg-black text-white flex items-center justify-center text-sm font-bold uppercase shadow-sm">
                    {userInitial}
                  </span>
